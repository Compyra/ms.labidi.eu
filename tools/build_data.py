...
SW_STATIC = ["./", "404.html", "style.css", "search.js", "app.js",
             "manifest.webmanifest", "icons/icon.svg", "icons/icon-192.png",
             "icons/icon-512.png", "icons/icon-maskable-512.png"]


def sync_shell_versions():
    """Single-source the ?v= version: rewrite index/404 tokens, emit sw.js."""
    version = (ROOT / "content" / "version.txt").read_bytes().decode("ascii").strip()
    if not version.isdigit():
        errors.append(f"version.txt: not a number: {version!r}")
        return
    token = f"?v={version}".encode("ascii")
    for name in ("index.html", "404.html"):
        path = ROOT / name
        path.write_bytes(re.sub(rb"\?v=\d+", token, path.read_bytes()))
    assets = []
    for entry in SW_STATIC:
        versioned = entry in ("style.css", "search.js", "app.js")
        if entry.startswith("icons/") and not (ROOT / entry).exists():
            continue
        assets.append(f"{entry}?v={version}" if versioned else entry)
    assets += [f"data/{p.name}?v={version}" for p in sorted(DATA.glob("*.js"))]
    template = (ROOT / "tools" / "sw_template.js").read_bytes().decode("utf-8")
    body = template.replace("__CACHE__", f"mshub-v{version}").replace(
        "__ASSETS__", ",\n".join(f'  "{a}"' for a in assets))
    (ROOT / "sw.js").write_bytes(
        (f"/* generated by tools/build_data.py from tools/sw_template.js; "
         f"do not edit. */\n{body}").encode("utf-8"))
    return version
...
def main():
    ...
    version = sync_shell_versions()
    if errors:
        for e in errors:
            print(f"ERROR: {e}")
        return 1
    print(f"ok: {len(records)} records from {upstream_count} upstream rows; "
          f"{len(synonyms)} synonyms; libraries: {libraries or 'none'}; v={version}; "
          "per subject: "
