...
Prep DONE 2026-08-26: brand PNGs generated by `tools/make_icons.py` (Pillow: 192/512
...
`sw.js` generation from `tools/sw_template.js` inside `build_data.py` (precache
enumerates `data/*.js` from disk, so a new data file can never be forgotten). Both
phase-8 test gates flipped to enforcing; the suite runs with zero skips and zero
pendings (39 python, 66 browser).

- [ ] Lighthouse a11y/SEO/best-practices green on the prod build.
...
- [ ] Family registry updates: `labidi.eu/js/projects.js` (all four language descs),
      `labidi.eu/todo-rami.md`.
- [ ] README final polish: attribution/NOTICE wording pass.
